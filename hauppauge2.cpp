...
bool ListDevs(void)
{
    ...
    for (Idev = devs.begin(); Idev != devs.end(); ++Idev)
    {
        cout << "[Bus: " << static_cast<int>(get<4>(*Idev))
             << " Port: " << static_cast<int>(get<5>(*Idev)) << "]  " << hex
             << get<0>(*Idev) << ":0x" << get<1>(*Idev) << " " << get<2>(*Idev)
             << " " << get<3>(*Idev) << endl;
...
int main(int argc, char * argv[])
{
    ...
    cmd_line.add_options()("version", "Display the version number")(
        "help,h", "Produce this help message.")(
        "config,c", po::value<std::string>(),
        "Config file")("list,l", "List detected devices.");

    po::options_description config{"Configuration options"};
    config.add_options()("v", po::value<int>()->implicit_value(1),
        ...
        ("boost,b", "Boost audio input gain.")(
            ...
            "29=MAX)")("videobitrate,r",
                       po::value<int>()->default_value(8000000),
                       "Video bitrate")(
            ...
            "Mininum VBR bitrate")("maxvbrrate,M",
                                   po::value<int>()->default_value(20000000),
                                   "Maximum VBR bitrate")(
            ...
            "Flip interlaced fields. Usually desireable, but MBAFF may look "
            "better with this off.")(
            "profile,p", po::value<int>()->default_value(4),
            "H.264 profile "
            ...
            "500=LEVEL_5, 510=LEVEL_5.1)")("bframes,B",
        ...
        ("logpath", po::value<string>(),
         ...
         "logpath with filenames in the format: hauppauge2-<serial#>.pid.log")(
            ...
            "levels will be discarded. In descending order: emerg, alert, "
            "crit, "
    ...
    if (vm.count("serial"))
    {
        ...
        {
            CRITLOG << "Device " << params.serial << " not found on USB bus.\n";
            ListDevs();
            return -1;
        }
        CRITLOG << "Initializing [Bus: " << bus << ", Port: " << port << "] "
                << params.serial;
    }

    if (vm.count("input"))
        params.videoInput =
            static_cast<_HAPI_VIDEO_CAPTURE_SOURCE>(vm["input"].as<int>());
    if (vm.count("audio"))
        params.audioInput =
            static_cast<_HAPI_AUDIO_CAPTURE_SOURCE>(vm["audio"].as<int>());
    if (vm.count("codec"))
        params.audioCodec =
            static_cast<HAPI_AUDIO_CODEC>(vm["codec"].as<int>());
    ...
    if (vm.count("audiobitrate"))
        params.audioBitrate =
            static_cast<_HAPI_AUDIO_BITRATE>(vm["audiobitrate"].as<int>());
    ...
    if (vm.count("videoratecontrol"))
        params.videoRateControl =
            static_cast<_HAPI_RATE_CONTROL>(vm["videoratecontrol"].as<int>());
    ...
    if (vm.count("videocodingmode"))
        params.videoCodingMode =
            static_cast<_HAPI_CODING_MODE>(vm["videocodingmode"].as<int>());
    if (vm.count("flipfields"))
        params.flipFields = vm["flipfields"].as<bool>();
    if (vm.count("profile"))
        params.videoProfile =
            static_cast<_HAPI_VIDEO_PROFILE>(vm["profile"].as<int>());
    if (vm.count("level"))
        params.videoH264Level =
            static_cast<_HAPI_VIDEO_H264_LEVEL>(vm["level"].as<int>());
    if (vm.count("bframes"))
    {
    ...
    if (vm.count("videolatency"))
        params.videoLatency =
            static_cast<_HAPI_LATENCY>(vm["videolatency"].as<int>());
    if (vm.count("aspect"))
    ...
    params.transcode =
        (vm.count("transcode")) ? vm["transcode"].as<bool>() : false;
    params.upmix = (vm.count("upmix")) ? vm["upmix"].as<bool>() : false;

    if (params.mythtv)
    {
        string desc = vm.count("description") ? vm["description"].as<string>()
                                              : params.serial;
    ...
    else
    {
        ...
        if (params.transcode)
        {
            transcoder = new Transcoder(params.output, nullptr, params.upmix);
            transcoder->Resume(); // It starts off paused
            params.output = std::string(); // reset this so HauppaugeDev doesn't
                                           // also write to it
        }
        ...
        if (!dev.Open(usbio, (params.audioCodec == HAPI_AUDIO_CODEC_AC3),
                              (params.transcode
                                   ? (DataTransfer::callback_t *)(*transcoder)
                                   : nullptr)))
        {
            usbio.Close();
            return -4;
